        ...
        if not git_update_all.is_ignore(repo_path):
            ...
            for remote_name, remote_info_dict in remote_info_dict_dict.items():
                ...
                if self.is_target(remote_url):
                    ...
                    if not self.b_arm:
                        git_util.logging.info(remote_name, remote_url, ssh_url)
                        git_util.logging.info(git_cmd)
                        git_util.logging.info(git_cmd_push)
                    else:
                        git_util.logging.info("** updating %r **" % repo_path)
                        git_util.logging.info(remote_name, remote_url)
                        git_util.git(git_cmd)
                        git_util.git(git_cmd_push)
                    os.chdir(self.start_path)
                elif remote_url is not None:
                    if ('@github.com' in remote_url) and ('http' in remote_url):
                        git_util.logging.info('skipping remote_url = %s' % remote_url)
